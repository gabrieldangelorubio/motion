...
- EL ZOOM SE CALCULA POR ESCENA CON LA CAJA DEL CONTENIDO, no es un valor fijo: alto_contenido = desde el tope del título hasta el borde inferior del elemento más bajo (chips, avatares, tarjetas laterales incluidos); zoom_max = alto_render / (alto_contenido × 1.05); zoom = min(ancho_render / ancho_pantalla, zoom_max); centro y = medio de esa caja. Con contenido de 840 px en un render de 1080, el zoom baja a 1.20–1.25 aunque la página pida 1.33 — si no, los chips de abajo o el título de arriba quedan cortados (la auditoría lo marcó tres veces en la otra versión).
...
Lo que salió MAL y no se repite: escribí «elevarDesenfocado» cuatro veces y no existe como entrada — el nombre correcto es subirDesenfocado; y tres encuadres salieron cortos (logo y garabato del hero, chips del plan, título de la caída) hasta que apliqué la fórmula de zoom por contenido de arriba.`;
...
- El zoom de cada escena sale de la CAJA DEL CONTENIDO (tope del título → borde inferior del elemento más bajo): zoom = min(ancho_render/ancho_pantalla, alto_render/(alto_contenido × 1.05)), centro y = medio de esa caja. Ningún hold es estático: push-in de +3 a +6 % con entradaSalidaSine durante el hold.
