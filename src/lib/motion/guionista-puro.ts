...
- El zoom de cada escena sale de la CAJA DEL CONTENIDO (tope del título → borde inferior del elemento más bajo, chips y tarjetas laterales incluidos) y de los MÁRGENES SEGUROS: el contenido ocupa como mucho el 80 % del cuadro (10 % libre por lado, zona de título), así el push-in del hold (+3 a +6 %) sigue dentro del 5 % que la auditoría exige (zona de acción). zoom = min(ancho_render × 0.8 / ancho_contenido, alto_render × 0.8 / alto_contenido), centro = el de esa caja. Si la pantalla es del color del fondo de la pieza, abrí el zoom sin miedo (abrir más allá de la página no muestra vacío); si no, no bajes de ancho_render/ancho_pantalla y encuadrá por secciones. Nada del contenido a menos del 5 % del cuadro de cada borde: se corta en pantalla. Ningún hold es estático: push-in de +3 a +6 % con entradaSalidaSine durante el hold.
